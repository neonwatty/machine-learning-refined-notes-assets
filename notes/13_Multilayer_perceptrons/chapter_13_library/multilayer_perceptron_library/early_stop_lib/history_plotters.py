...
class Setup:
    ...
    def plot_cost_histories(self, train_cost_histories, valid_cost_histories, start):
        ...
        for c in range(len(train_cost_histories)):
            ...
            ax.plot(
                np.arange(start, len(train_history), 1), train_history[start:], linewidth=3 * (0.8) ** (c), color=self.colors[0], label="train cost"
            )
            ...
            if np.size(valid_cost_histories) > 0:
                ...
                ax.plot(
                    np.arange(start, len(val_history), 1), val_history[start:], linewidth=3 * (0.8) ** (c), color=self.colors[1], label="test cost"
                )
    ...
    def plot_cost_count_histories(self, train_cost_histories, train_count_histories, valid_cost_histories, valid_count_histories, start):
        ...
        for c in range(len(train_cost_histories)):
            ...
            ax2.plot(
                np.arange(start, len(train_count_history), 1),
                train_count_history[start:],
                linewidth=3 * (0.8) ** (c),
                color=self.colors[0],
                label="train",
            )
            ...
            if np.size(valid_cost_histories) > 0:
                ...
                ax2.plot(
                    np.arange(start, len(valid_count_history), 1),
                    valid_count_history[start:],
                    linewidth=3 * (0.8) ** (c),
                    color=self.colors[1],
                    label="validation",
                )
