...
class visualizer:
    ...
    def svm_comparison(self):
        ...
        for i in range(3):
            # run newton's method
            w_hist = opt.gradient_descent(
                g=self.margin_perceptron, w=np.random.randn(np.shape(self.x)[1] + 1, 1), max_its=50, steplength_rule="diminishing"
            )
