    ...
    def gradient_descent(self, g, w_hist, savepath, **kwargs):
        ...
        def animate(t):
            ...
            if k == 0:
                ...
                g_val = g(w_val)
                ax.scatter(
                    w_val, g_val, s=90, color=colorspec[k], edgecolor="k", linewidth=0.5 * (1 / (float(k) + 1)) ** (0.4), zorder=3, marker="X"
                )  # evaluation on function
        ...
        def animate(k):
            ...
            if k > 0:
                ...
                for j in range(len(self.w_hist)):
                    ...
                    grad_val = self.grad(w_val)
                    ax1.scatter(
                        w_val[0], w_val[1], g_val, s=90, color=self.colorspec[j], edgecolor="k", linewidth=0.7, zorder=3
                    )  # plot point of tangency
