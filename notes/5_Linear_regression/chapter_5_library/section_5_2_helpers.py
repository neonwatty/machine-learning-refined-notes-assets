...
class linear_regression_visualizer:
    ...
    def plot_data(self):
        ...
        # Turn off axes for ax1 and ax3
        ax1.axis("off")
        ax3.axis("off")
        # ax2.set_position([0.1, 0.3, 0.9, 0.6]) 
    ...
    def scatter_pts(self, ax):
        ...
        if np.shape(self.x)[1] == 2:
            ...
            # initialize points
            ax.scatter(self.x[:, 0][:, np.newaxis], self.x[:, 1][:, np.newaxis], self.y, s=40, color="k", edgecolor="w", linewidth=0.9)
            ...
            ax.set_ylabel(r"$x_2$", rotation=0, fontsize=12, labelpad=2)
